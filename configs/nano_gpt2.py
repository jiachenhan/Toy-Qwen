...
配置层级（优先级低 → 高）：
  架构定义   ModelConfig   — 改了就是另一个模型，不通过 CLI 修改
  训练配方   TrainConfig   — 偶尔调整，可通过 CLI --lr / --batch-size 覆盖
  实验变量   CLI only      — --impl / --device，每次 run 可能不同，不写在此文件
"""
...
class ModelConfig:
    ...
    dropout: float = 0.0
    # 由 CLI --impl 设置，写在这里仅用于序列化到 config.json
